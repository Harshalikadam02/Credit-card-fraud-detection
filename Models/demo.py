...
# Check if the initial form has been validated as normal
if 'initial_form_normal' not in st.session_state:
...
if not st.session_state.initial_form_submitted:
    ...
    # Process initial form submission
    if submit_button:
        ...
        # Check if the actual travel time is less than the estimated travel time
        if time_difference < travel_time_estimate_timedelta:
            ...
            # If the transaction appears normal, update the session state and ask for domain name and transaction amount
            ...
            #st.session_state.current_timestamp_dt = datetime.combine(date_input, time_input)

        # # Update session state to mark the initial form as submitted
        # st.session_state.initial_form_submitted = True

# Once the initial form has been submitted and validated as normal, prompt the user for domain name and transaction amount
if st.session_state.initial_form_submitted and st.session_state.initial_form_normal:
    ...
    # If the user submits the anomaly check form
    if submit_anomaly_button:
        # Call the train_model function to get the model and scaler
        ...
        # Call the predict_anomaly function to determine if the new data is an anomaly
    ...
    if back_button:
        ...
        # Refresh the page to go back to the initial form
